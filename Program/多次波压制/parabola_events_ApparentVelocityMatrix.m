function [d,h,t,ApparentVelocityMatrix] = parabola_events_ApparentVelocityMatrix(dt,f0,tmax,h,tau,p,amp,snr,L)
...
%
 ...
 %%
 ApparentVelocityMatrix=zeros(nt,nh);
 width=25;
 %%
...
%     Shift = exp(-i*w*(  sqrt(tau(k)^2 + (h/v(k)).^2) - delay));
   Shift = exp(-i*w*(  tau(k) + p(k)*h.^2 - delay));
   D(ifreq,:) = D(ifreq,:) +amp(k)* W(ifreq)*Shift;
   for kv=1:nh
    tv=floor( (tau(k) + p(k)*h(kv).^2)/dt);
    ApparentVelocityMatrix(tv-width:tv+width,kv)=1./(2*p(k)*h(kv));
   end
