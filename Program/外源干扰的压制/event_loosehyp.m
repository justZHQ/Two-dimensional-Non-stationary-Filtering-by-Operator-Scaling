function amat=event_loosehyp(amat,t,x,tnot,xnot,v,amp,flag,aper,m,w,kw)
...
for k=1:nc
    ...
    if aper==inf
        if(xoff<m)
            ...
            tk = sqrt(tnot^2+(xoff/(v-w*(abs(xoff))/dx+kw))^2);
        end
    else
        tk = sqrt(tnot^2+(xoff/(v-w*(abs(xoff)-aper)/dx+kw))^2);
    end
		a=amp;
		if(flag==2)
			a = tnot*a/tk;
